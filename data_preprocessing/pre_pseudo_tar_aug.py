...
u_train_f= open(os.path.join("./data/", args.ds, 'unlabel_tar_train.tsv'),'r')
u_train_f_lines=u_train_f.readlines()

for i in range(len(u_train_f_lines)):
    # u_train_senti_list.append(u_train_f_lines[i].strip().split('\t')[2])
    u_train_f_lines[i]=u_train_f_lines[i].strip().split('\t')[1]

if(args.aug_mode=='tfidf'):
    word,word_prob,sentence_prob=create_tfidf_dic(u_train_f_lines,args.uda_p)

f=open(os.path.join('./data',args.ds,'aug_unlabel_tar_train.tsv'),'w')
f.write("s_id\tsentence\taug_sentence\tsentiment\n")
aug_size=0
for i in trange(len(u_train_f_lines),desc='write aug'):
    u_train_text=u_train_f_lines[i]
    # u_train_senti=u_train_senti_list[i]
    if(i==0):continue
    ...
    # f.write('semi\t'+u_train_text+'\t'+aug_text+'\t'+u_train_senti+'\n')
    f.write('semi\t'+u_train_text+'\t'+aug_text+'\n')
    aug_size+=1
f.close()
u_train_f.close()
...
"""
apply data augmentation on unlabeled train text (sentences)
UNK and SYN is used in the paper
but in the implementation, mix is used 
"""